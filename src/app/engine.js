/**
 * @fileOverview Puts all the pieces together and runs the game
 * @authors Noel Noche
 * @version 1.0.0
 */
...
var activeGem = false; // Ensures only one gem appears at one time
...
function update(dt, nowArg) {
  ...
  currentTime = 180 - parseInt((nowArg - startTime) / 1000);
  ...
  // Generates new bugs when they pass the limits of the game grid
  for (i = 0, len = allEnemies.length; i < len; i++) {
    ...
    if (allEnemies[i].name !== 'purple' && allEnemies[i].x > canvas.width || allEnemies[i].x < -101) {
  ...
  // When player object (character) reaches top, a new character is generated
  if (playerInstance.done === true) {
    doneChars.push(playerInstance);
    
    // Update character remaining list
    charIndex = allChars.indexOf(playerInstance.sprite);
    ...
    if (activeGem === true) {
      ...
      if (gemInstance.taken) {
        if (soundOn === true) {
          playSound('start');
        }
        if (gemInstance.bonusLife === true) {
          addHeart();
        }
        else {
          score += gemInstance.points
    ...
    if (doneChars.length === 5) {
      minScore = currentTime * PTS_PER_MIN;
      score += minScore;
      updateHudScore(score);
      gameLevel += 1;
      ...
      gemCols.forEach(function(colX) {
        if (playerInstance.x === colX) {
          gemIndex = gemCols.indexOf(colX);
          gemCols.splice(gemIndex, 1);
        }
      });
...
function updateEntities(dt) {
  ...
  if (gemInstance !== null && gemInstance.activeTimer === true && activeGem === true) {
    gemInstance.updateTimer(7);
    
    if (gemInstance.taken === true) {
      gemInstance.activeTimer = false;
    }
  }
  else {
    activeGem = false;
...
function renderEntities() {
  ...
  // Renders the rock type Array (the water array is handled by renderMap)
  for (i = 0, len = rocksArray.length; i < len; i++) {
