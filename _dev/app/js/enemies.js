...
var RedBug1 = function() {
  Enemy.call(this);
  this.name = 'red1';
  this.sprite = 'assets/images/bug-red-right.png';
  this.fromLeft = null;
};
...
// Yellows appears from random rows and moves in random speeds
var YellowBug = function() {
  Enemy.call(this);
  this.name = 'yellow';
  this.sprite = 'assets/images/bug-yellow-right.png';
};

YellowBug.prototype = Object.create(Enemy.prototype);
YellowBug.prototype.constructor = YellowBug;

// Green bugs can change rows at random times
var GreenBug = function() {
  Enemy.call(this);
  this.name = 'green';
  this.sprite = 'assets/images/bug-green-right.png';
  this.rowShift = false;
};
...
BlueBug.prototype.update = function(dt, player) {
  ...
  if (this.x < player.x) {
    this.sprite = 'assets/images/bug-blue-right.png';
    this.x = this.x + this.speed * dt;
  }

  if (this.x > player.x) {
    this.sprite = 'assets/images/bug-blue-left.png';
    this.x = this.x - this.speed * dt;
  }
};

// Purples moves on the top row then targets the player when y-positions overlap
var PurpleBug = function() {
  Enemy.call(this);
  this.name = 'purple';
  this.sprite = 'assets/images/bug-purple-right.png';
  this.locked = false;
};
...
function generate_bug(type, redIndex) {
  ...
  switch (type) {
    ...
    case 'yellow':
      bug = new YellowBug();
      bug.y = ROWS[RandTools.gen_rand_index(4, 1)];
      bug.speed = SPEEDS[RandTools.gen_rand_index(SPEEDS.length, 0)];
      bug.fromLeft = BOOLS[RandTools.gen_rand_index(BOOLS.length, 0)];
      break;
    case 'green':
      bug = new GreenBug();
      bug.y = ROWS[RandTools.gen_rand_index(2, 2)];
      bug.speed = 150;
      bug.fromLeft = BOOLS[RandTools.gen_rand_index(BOOLS.length, 0)];
      bug.rowShift = BOOLS[RandTools.gen_rand_index(BOOLS.length, 0)];
      break;
    case 'blue':
      bug = new BlueBug();
      bug.y = blueRow;
  ...
  if ((bug.name).match(/^red[1-4]/)) {
    ...
    if (bug.fromLeft === true) {
      bug.x = COLS[RandTools.gen_rand_index(4, 1)] + (bug.w + 153) * redIndex;
    } else {
      bug.sprite = 'assets/images/bug-red-left.png';
      bug.x = COLS[RandTools.gen_rand_index(4, 1)] - (bug.w + 153) * redIndex;
    ...
    if (bug.fromLeft === true) {
      ...
      if ((bug.name).match(/^red[1-4]/) === null) {
        bug.sprite = 'assets/images/bug-' + bug.name + '-left.png';
      }
